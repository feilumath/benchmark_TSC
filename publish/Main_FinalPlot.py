...
def main(*,root, date, Majorind, Toycase=False):
    ...
    for i in range(name_app_list.__len__()):
        ...
        for j in range(num_x_toplot):
            ...
            if Toycase:
                mid_auc_errorbar_np[0, j] = mid_auc_mean_np[j] - np.sort(mid_mid_auc_np)[0]
                mid_auc_errorbar_np[1, j] = -mid_auc_mean_np[j] + np.sort(mid_mid_auc_np)[-1]
            else:
    ...
    def plot_here(res_load, x_toplot, *, name_app_list, name_app_list_saved, time_str, Sub_Results_path, str_additional):
        ...
        plt.plot(x_toplot, getattr(res_load, 'auc_mean' + '_hiddentrue' + '_np'), c='black', lw=3, ls='-', label='LRT hidden truth')
        plt.plot(x_toplot, getattr(res_load, 'auc_mean' + '_simpleapprox' + '_np'), c=color_list_saved[3], lw=3.5, ls=linestyle_tuple_list_saved[4][1], label='LRT numerical')
        for i in range(name_app_list.__len__()):
        ...
        # plotforsave_str = 'C:/Users/zzh19/Desktop/Plots/' + case_name + '.pdf'
